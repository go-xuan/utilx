...
// AddCronTask 添加定时任务
func (c *CronScheduler) AddCronTask(task *Cron) *CronScheduler {
	c.mutex.Lock()
...
// Get 获取定时任务
func (c *CronScheduler) Get(id string) *Cron {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	if task, ok := c.tasks[id]; ok {
		task.entry = c.cron.Entry(task.entry.ID)
